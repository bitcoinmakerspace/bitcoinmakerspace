use wasm_bindgen::JsCast;
use yew::prelude::*;
use yewdux::prelude::*;

use crate::{
    library::ColorMode,
    store::{set_color_mode, GlobalStore},
};

#[function_component(ColorModeSwitch)]
pub fn color_mode_switch() -> Html {
    let (store, dispatch) = use_store::<GlobalStore>();

    let color_mode = store.color_mode.clone();
    let onclick = Callback::from(move |_| {
        let document = web_sys::window().unwrap().document().unwrap();
        let element = document.query_selector("html").unwrap().unwrap();
        let html = element.dyn_into::<web_sys::HtmlElement>().unwrap();

        let disp: Dispatch<GlobalStore> = dispatch.clone();

        if store.color_mode.is_none() {
            let os_dark = web_sys::window()
                .unwrap()
                .match_media("(prefers-color-scheme: dark)")
                .unwrap()
                .unwrap();
            if os_dark.matches() {
                html.class_list().remove_1("dark").unwrap();
                html.class_list().add_1("light").unwrap();
                set_color_mode(Some(ColorMode::Light), disp)
            } else {
                html.class_list().remove_1("light").unwrap();
                html.class_list().add_1("dark").unwrap();
                set_color_mode(Some(ColorMode::Dark), disp)
            }
        } else if let Some(ColorMode::Light) = store.color_mode {
            html.class_list().remove_1("light").unwrap();
            html.class_list().add_1("dark").unwrap();
            set_color_mode(Some(ColorMode::Dark), disp)
        } else if let Some(ColorMode::Dark) = store.color_mode {
            html.class_list().remove_1("dark").unwrap();
            html.class_list().add_1("light").unwrap();
            set_color_mode(Some(ColorMode::Light), disp)
        }
    });

    html! {
        <button onclick={onclick} class={classes!("buttons", "textbase")}>
            {if color_mode.is_none() {
                if web_sys::window()
                .unwrap()
                .match_media("(prefers-color-scheme: dark)")
                .unwrap()
                .unwrap().matches() {
                    html! {
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M12 3v2.25m6.364.386l-1.591 1.591M21 12h-2.25m-.386 6.364l-1.591-1.591M12 18.75V21m-4.773-4.227l-1.591 1.591M5.25 12H3m4.227-4.773L5.636 5.636M15.75 12a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0z" />
                        </svg>
                    }
                } else {
                    html! {
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M21.752 15.002A9.718 9.718 0 0118 15.75c-5.385 0-9.75-4.365-9.75-9.75 0-1.33.266-2.597.748-3.752A9.753 9.753 0 003 11.25C3 16.635 7.365 21 12.75 21a9.753 9.753 0 009.002-5.998z" />
                        </svg>
                    }
                }
            } else if color_mode == Some(ColorMode::Light) {
                html! {
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M21.752 15.002A9.718 9.718 0 0118 15.75c-5.385 0-9.75-4.365-9.75-9.75 0-1.33.266-2.597.748-3.752A9.753 9.753 0 003 11.25C3 16.635 7.365 21 12.75 21a9.753 9.753 0 009.002-5.998z" />
                    </svg>
                }
            } else {
                html! {
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 3v2.25m6.364.386l-1.591 1.591M21 12h-2.25m-.386 6.364l-1.591-1.591M12 18.75V21m-4.773-4.227l-1.591 1.591M5.25 12H3m4.227-4.773L5.636 5.636M15.75 12a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0z" />
                    </svg>
                }
            }}
        </button>
    }
}
